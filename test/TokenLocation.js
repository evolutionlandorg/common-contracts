...
contract('Token Location Test', async(accounts) => {
    ...
    it('test encode and decode in 100M', async() => {
        ...
        let locationId = await tokenLocation.encodeLocationIdHM.call(-49, 49);
        let location = await tokenLocation.decodeLocationIdHM.call(locationId);
        assert.equal(location[0].toNumber(), -49);
        assert.equal(location[1].toNumber(), 49);

        let locationId1 = await tokenLocation.encodeLocationIdHM.call(1, -1);
        console.log("locationId1... " + locationId1);
        let location1 = await tokenLocation.decodeLocationIdHM.call(locationId1);
        assert.equal(location1[0].toNumber(), 1);
        assert.equal(location1[1].toNumber(), -1);
